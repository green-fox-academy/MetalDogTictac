...
<html>
    <head>
        <style>body{background: skyblue; font-family: verdana; color: #fff; padding: 30px;}
    ...
    <body>
        ...
        <h2>Hobbies</h2>
        <ul>
            <% data.hobbies.forEach(function(item){ %>  //when we output JS, we only use <%, with no = sign

            <li><%= item %></li>    //when we output data, we must use <%=
            <% }); %>
        </ul>
    </body>
</html>